...
function run(command, cli) {
  switch (command) {
    case 'build':
      return buildTsRun(cli);
    case 'link':
      return npmLinkRun(cli);
    case 'version':
      return npmVersionRun(cli);
    case 'dist-tag':
      return npmDistTagRun(cli);
    case 'publish':
      return npmPublishRun(cli, {buildCommand});
    default:
      throw new Error(`You are using unknown command '${command}', 
